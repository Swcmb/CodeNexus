...
class TestQASystemSimpleProperties:
    """问答系统简化属性测试类"""
    ...
    def create_simple_graph(self, element_name="TestClass"):
        """创建一个简单的测试图谱"""
        element = CodeElement(
            name=element_name,
            type="class",
    ...
    @given(st.text(min_size=3, max_size=20).filter(lambda x: x.isalnum() and x.isascii() and x.isalpha()))
    ...
    def test_question_understanding_property(self, class_name):
        ...
        **验证需求: 需求 5.1**
        ...
        """
        # 过滤无效的类名
        assume(class_name.isalpha() and len(class_name) >= 3)
    ...
    @given(st.text(min_size=3, max_size=15).filter(lambda x: x.isalnum() and x.isascii() and x.isalpha()))
    @settings(max_examples=10, deadline=None)
    def test_code_location_property(self, element_name):
        ...
        **验证需求: 需求 5.2**
        ...
        """
        assume(element_name.isalpha() and len(element_name) >= 3)
        
        # 创建包含指定元素的图谱
        graph = self.create_simple_graph(element_name)
    ...
    @given(st.lists(st.text(min_size=2, max_size=8).filter(lambda x: x.isalnum() and x.isascii() and x.isalpha()), 
    ...
    def test_confidence_calculation_property(self, keywords):
        ...
        **验证需求: 需求 5.4**
        ...
        """
        # 过滤无效关键词
        valid_keywords = [kw for kw in keywords if kw.isalpha() and len(kw) >= 2]
        assume(len(valid_keywords) > 0)
        
        # 创建一个匹配第一个关键词的代码元素
        target_keyword = valid_keywords[0]
        element = CodeElement(
            name=target_keyword,
            type="class",
            file_path="src/test.py",
