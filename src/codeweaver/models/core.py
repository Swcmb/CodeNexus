...
class CodeGraph:
    ...
    def remove_node(self, node_id: str) -> None:
        ...
        # 移除相关的边
        edges_to_remove = node.incoming_edges.union(node.outgoing_edges)
    ...
    def remove_edge(self, edge_id: str) -> None:
        ...
        if edge.source_node_id in self.nodes:
            self.nodes[edge.source_node_id].remove_outgoing_edge(edge_id)
        if edge.target_node_id in self.nodes:
            self.nodes[edge.target_node_id].remove_incoming_edge(edge_id)
        ...
        if direction in ("both", "outgoing"):
            for edge_id in node.outgoing_edges:
                if edge_id in self.edges:
                    neighbors.add(self.edges[edge_id].target_node_id)
        
        if direction in ("both", "incoming"):
            for edge_id in node.incoming_edges:
                if edge_id in self.edges:
                    neighbors.add(self.edges[edge_id].source_node_id)
